...
function filtering($db,$columnName,$filter,$tableName,$find,$sorting){
   ...
   if($filter == "All"){
      $query = "SELECT ".$columnName." FROM $tableName WHERE Category = 'Game' "." ORDER BY ".$sorting;
   }else {
    $query = "SELECT ".$columnName." FROM $tableName WHERE (Category = 'Game' AND type = '".$filter."') ORDER BY ".$sorting;}
