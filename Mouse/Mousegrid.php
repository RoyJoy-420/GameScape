...
function filtering($db,$columnName,$filter,$tableName,$find,$sorting){
   ...
   if($filter == "All"){
      $query = "SELECT ".$columnName." FROM $tableName WHERE Category = 'Mouse' "." ORDER BY ".$sorting;
   }else {
    $query = "SELECT ".$columnName." FROM $tableName WHERE (Category = 'Mouse' AND connectivity = '".$filter."') ORDER BY ".$sorting;}
