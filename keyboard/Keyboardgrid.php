...
function filtering($db,$columnName,$filter,$tableName,$find,$sorting){
   ...
   if($filter == "All"){
      $query = "SELECT ".$columnName." FROM $tableName WHERE Category = 'Keyboard' "." ORDER BY ".$sorting;
   }else {
    $query = "SELECT ".$columnName." FROM $tableName WHERE (Category = 'Keyboard' AND connectivity = '".$filter."') ORDER BY ".$sorting;}
...
function result($db, $tableName, $columns,$sorting,$filter){
    ...
    //$query = "SELECT ".$columnName." FROM $tableName"." ORDER BY ".$sorting;
    $result = $db->query($query);
    return $result;
}
